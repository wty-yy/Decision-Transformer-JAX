...
def parse_args_and_writer(input_args=None, with_writer=True) -> tuple[argparse.Namespace, SummaryWriter]:
  ...
  ### Gobal ###
  parser.add_argument("--name", type=str, default="RADT_wty_v0.2")
  ...
  parser.add_argument("--n-token", type=int, default=60)
  ...
  assert args.n_token % 2 == 0, f"n_token must be divided by 3, since n_step = n_token / 3"
  args.n_step = args.n_token // 2
  ...
  args.run_name = f"{args.name}__{args.game}__{args.seed}__{time.strftime(r'%Y%m%d_%H%M%S')}"
