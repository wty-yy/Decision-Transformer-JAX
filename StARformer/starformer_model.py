...
class StARformer(nn.Module):
  ...
  def __call__(self, s, a, r, timestep, train = True):
    ...
    xg = nn.Sequential([  # (B, N, 84, 84, 4) -> (B, l, Ng) global token
    ...
    a = Embed(cfg.n_vocab+1, nl)(a).reshape(B, N, 1, nl)  # (B, N) -> (B, N, Nl)
    ...
    ### Concatenate Group ###
    xl = jnp.concatenate([a, s, r], 2)  # (B, N, P + 1 + (1), Nl)
...
if __name__ == '__main__':
  ...
  # star(offical) parameters: 14373220
  ...
  gpt_cfg = StARConfig(n_vocab=n_vocab, n_step=n_step, max_timestep=max_timestep, patch_size=patch_size, mode=mode)
  print(dict(gpt_cfg))
